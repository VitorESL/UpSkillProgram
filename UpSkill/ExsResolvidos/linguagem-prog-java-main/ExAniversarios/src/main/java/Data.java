...
public class Data implements Comparable<Data> {
    ...
    public void setMes(int mes) throws MesInvalidoException {
        if (mes < 1 || mes > 12) {
            throw new MesInvalidoException("Meses são entre 1(Janeiro) e 12(Dezembro), introduziu " + mes);
        }
    ...
    public void setDia(int dia) throws DiaInvalidoException {
        int diasDesteMes = this.mes.numeroDeDias(this.ano);
        if (dia > diasDesteMes) {
            throw new DiaInvalidoException("O mês de " + this.mes + " só tem " +
                    diasDesteMes + "dias. Introduziu " + dia);
        } else if (dia < 1) {
            throw new DiaInvalidoException("Não podem existir dias inferiores a 1. Introduziu" + dia);
        }
        this.dia = dia;
    }
    ...
    public Data() throws MesInvalidoException, DiaInvalidoException {
        setAno(ANO_POR_OMISSAO);
        setMes(MES_POR_OMISSAO);
        setDia(DIA_POR_OMISSAO);
    }
     ...
     */
    public Data(Data outraData) throws DiaInvalidoException {
        setAno(outraData.ano);
        setMes(outraData.mes);
        setDia(outraData.dia);
    }
     ...
     */
    public final void setData(int ano, int mes, int dia) throws DiaInvalidoException, MesInvalidoException {
        this.setAno(ano);
        this.setMes(mes);
        this.setDia(dia);
    }
     ...
     *         parâmetro com ano, mês e dia.
     */
    public int diferenca(int ano, int mes, int dia) throws DiaInvalidoException, MesInvalidoException {
        int totalDias = contaDias();
     ...
     */
    public static Data dataAtual() throws DiaInvalidoException, MesInvalidoException {
