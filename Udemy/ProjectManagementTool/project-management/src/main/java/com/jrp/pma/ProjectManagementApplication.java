...
//			Case we want to use the id incrementor in the DB via java code (here commented, because it is bad practice to do so via java)
//			we need to change @GeneratedValue(strategy=GenerationType.(IDENTITY) to (Auto). Right now, IDENTITY is necessary so that when
//  		user creates new employee in the app, the id can be incremented without breaking the primary key, as it would not be recognised
//  		as AUTO with the next value for id and would just try to create again id=1. also if we want to define the sql schema without
//			hibernate we can do so by creating schema.sql in the resources (with the create tables and constraints etc) and add to the
//			applications.properties the command spring.jpa.hibernate.ddl-auto=none.
		};
	}*/
